...
function s3image(list, path, options) {
	...
	if (options.initial) {
		throw new Error('Invalid Configuration\n\n' + 'S3Image fields (' + list.key + '.' + path + ') do not currently support being used as initial fields.\n');
	}
 ...
 */
Object.defineProperty(s3image.prototype, 's3config', {
	get: function get() {
 ...
 */
s3image.prototype.addToSchema = function () {
   ...
   */
		...
		/**
   * Deletes the file from S3Image and resets the field
   *
   * @api public
   */
 ...
 */
s3image.prototype.hasFormatter = function () {
 ...
 * Deprecated
 */
s3image.prototype.inputIsValid = function (data) {
 ...
 */
s3image.prototype.updateItem = function (item, data, callback) {
	// eslint-disable-line no-unused-vars
	// TODO - direct updating of data (not via upload)
	if (data[this.paths.alt]) {
		item.set(this.paths.alt, data[this.paths.alt]);
	}
 ...
 * @return {Boolean}
 */
var validateHeader = function validateHeader(header, callback) {
 ...
 * @return {Boolean}
 */
var validateHeaders = function validateHeaders(headers, callback) {
	var _headers = [];
 ...
 * @return {Object}
 */
s3image.prototype.generateHeaders = function (item, file, callback) {
	var field = this;
 ...
 */
s3image.prototype.uploadFile = function (item, file, update, callback) {
 ...
 * - `field.paths.upload` in `req.files` (uploads the file to s3image)
 */
s3image.prototype.getRequestHandler = function (item, req, paths, callback) {
 ...
 */
s3image.prototype.handleRequest = function (item, req, paths, callback) {
